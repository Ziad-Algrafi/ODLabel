...
from chart_utils import input_chart, output_charts, close_matplotlib_figures
...
class MainWindow:
    def __init__(self, root):
        ...
        self.slice_size_entry = customtkinter.CTkEntry(self.left_frame, textvariable=self.slice_size_var, width=160)
        ...
        self.conf_slider = customtkinter.CTkSlider(self.left_frame, from_=0, to=1, number_of_steps=100, variable=self.conf_var, command=self.update_conf_label)
    ...
    def process_images(self, folder_path):
        try:
            ...
            self.clear_output()
            fig_format, fig_resolution, fig_quality, fig_colorspace = input_chart(folder_path, self.chart_colors)
            self.display_chart(fig_format, self.input_chart_frame, row=0, col=0)
            self.display_chart(fig_resolution, self.input_chart_frame, row=0, col=1)
            self.display_chart(fig_quality, self.input_chart_frame, row=1, col=0)
            self.display_chart(fig_colorspace, self.input_chart_frame, row=1, col=1)
            self.input_figures = [fig_format, fig_resolution, fig_quality, fig_colorspace]
        except Exception as e:
            self.show_error_message(str("No images in the selected folder"))
    ...
    def select_images_folder(self):
        ...
        if folder_path:
            ...
            self.check_run_button_state()
            self.root.after(0, self.process_images, folder_path)


    def update_input_chart(self, images_folder):
        self.clear_input_chart()
        
        def generate_format_chart():
            try:
                fig_format, _, _, _ = input_chart(images_folder, self.chart_colors)
                self.display_chart(fig_format, self.input_chart_frame, row=0, col=0)
            except Exception as e:
                self.show_error_message(str(e))
        
        def generate_resolution_chart():
            try:
                _, fig_resolution, _, _ = input_chart(images_folder, self.chart_colors)
                self.display_chart(fig_resolution, self.input_chart_frame, row=0, col=1)
            except Exception as e:
                self.show_error_message(str(e))
        ...
        def generate_quality_chart():
            try:
                _, _, fig_quality, _ = input_chart(images_folder, self.chart_colors)
                self.display_chart(fig_quality, self.input_chart_frame, row=1, col=0)
            except Exception as e:
                self.show_error_message(str(e))
        
        def generate_colorspace_chart():
            try:
                _, _, _, fig_colorspace = input_chart(images_folder, self.chart_colors)
                self.display_chart(fig_colorspace, self.input_chart_frame, row=1, col=1)
            except Exception as e:
                self.show_error_message(str(e))
        
        threading.Thread(target=generate_format_chart).start()
        threading.Thread(target=generate_resolution_chart).start()
        threading.Thread(target=generate_quality_chart).start()
        threading.Thread(target=generate_colorspace_chart).start()
    ...
    def on_worker_finished(self):
        train_detections, val_detections, num_train_images, num_val_images = self.worker_thread.get_detections()
        self.worker_thread = None
        ...
        try:
            self.update_output_charts(train_detections, val_detections, num_train_images, num_val_images)
        except Exception as e:
            self.show_error_message(str(e))

    def update_output_charts(self, train_detections, val_detections, num_train_images, num_val_images):
        if train_detections:
            train_fig_count, train_fig_conf, train_fig_labels = output_charts(train_detections, num_train_images)
        ...
        if val_detections:
            val_fig_count, val_fig_conf, val_fig_labels = output_charts(val_detections, num_val_images)
    ...
    def display_chart(self, fig, frame, row, col, colspan=1):
        ...
        if colspan > 1:
            for i in range(1, colspan):
                frame.grid_columnconfigure(col+i, weight=1)
