...
class ObjectDetectionWorker(QThread):
    ...
    def __init__(self, conf_threshold, model_path, images_folder,
                 output_directory, val_split, output_format, classes, device,
                 use_sahi, slice_size):
        super().__init__()
    ...
    def infer(self, model, img, conf_threshold, log_update, image_file):
        ...
        if results[0].boxes is not None:
            ...
            for box, class_id in zip(boxes, clss):
                ...
                detections.append((cls_id, x_scaled, y_scaled, w_scaled,
                                   h_scaled, class_name))
        else:
    ...
    def slice_and_infer(self, model, img, conf_threshold, log_update,
                        image_file, slice_size):
        annotated_frame = img.copy()
    ...
    def draw_boxes_on_image(self,
        ...
        for _, x_scaled, y_scaled, w_scaled, h_scaled, class_name in detections:
        ...
        if log_update:
            log_update.emit(
                f"Saved image with bounding boxes to {output_path}")

        return annotated_frame

    def save_yolo_format(self, detections, out_dir, image_file, img):
        result_txt_path = os.path.join(
            out_dir, f"{os.path.splitext(image_file)[0]}.txt")
        with open(result_txt_path, 'w') as txt_file:
            for cls_id, x_scaled, y_scaled, w_scaled, h_scaled, class_name in detections:
                ...
                txt_file.write(
                    f"{cls_id} {normalized_x} {normalized_y} {normalized_w} {normalized_h}\n"
                )

    def save_coco_format(self, detections, image_file, img, i, coco_output):
    ...
    def save_csv_format(self, detections, image_file, img, csv_writer):
        for cls_id, x_scaled, y_scaled, w_scaled, h_scaled, class_name in detections:
            csv_writer.writerow([
                class_name,
                ...
                float(w_scaled),
                float(h_scaled), image_file,
                int(img.shape[1]),
                int(img.shape[0])
            ])

    def save_xml_format(self, detections, out_dir, image_file, images_dir,
                        img):
        result_xml_path = os.path.join(
            out_dir, f"{os.path.splitext(image_file)[0]}.xml")
        ...
        for cls_id, x_scaled, y_scaled, w_scaled, h_scaled, class_name in detections:
            ...
            ET.SubElement(object_element,
                          'name').text = str(self.classes[cls_id])
            ...
            ET.SubElement(bndbox_element,
                          'xmin').text = str(int(x_scaled - w_scaled / 2))
            ET.SubElement(bndbox_element,
                          'ymin').text = str(int(y_scaled - h_scaled / 2))
            ET.SubElement(bndbox_element,
                          'xmax').text = str(int(x_scaled + w_scaled / 2))
            ET.SubElement(bndbox_element,
                          'ymax').text = str(int(y_scaled + h_scaled / 2))
    ...
    def run(self):
        ...
        for i, image_file in enumerate(image_files):
            ...
            if self.use_sahi:
                detections, annotated_frame = self.slice_and_infer(
                    model, img, self.conf_threshold, self.log_update,
                    image_file, self.slice_size)
            else:
                detections, annotated_frame = self.infer(
                    model, img, self.conf_threshold, self.log_update,
                    image_file)
            ...
            if i < num_val_images:
                ...
                if self.output_format == "CSV":
                    val_csv_file_path = os.path.join(val_labels_dir,
                                                     "annotations.csv")
                    with open(val_csv_file_path, 'a',
                              newline='') as val_csv_file:
                        val_csv_writer = csv.writer(val_csv_file)
                        if i == 0:
                            val_csv_writer.writerow([
                                'label_name', 'bbox_x', 'bbox_y', 'bbox_width',
                                'bbox_height', 'image_name', 'image_width',
                                'image_height'
                            ])
                elif self.output_format == "XML":
                    self.save_xml_format(detections, labels_dir, image_file,
                                         images_dir, img)
                ...
                if self.output_format == "CSV":
                    train_csv_file_path = os.path.join(train_labels_dir,
                                                       "annotations.csv")
                    with open(train_csv_file_path, 'a',
            ...
            if detections:
                output_img = self.draw_boxes_on_image(
                    annotated_frame, detections, img_path,
                    os.path.join(images_dir, image_file))
                if self.output_format == "YOLO":
                    self.save_yolo_format(detections, labels_dir, image_file,
                                          img)
                elif self.output_format == "COCO":
                    self.save_coco_format(detections, image_file, img, i,
                                          coco_output)
                elif self.output_format == "CSV":
                    if i < num_val_images:
                        self.save_csv_format(detections, image_file, img,
                                             val_csv_writer)
                    else:
                        self.save_csv_format(detections, image_file, img,
                                             train_csv_writer)
        ...
        if self.output_format == "COCO":
            if num_val_images > 0:
                val_coco_json_path = os.path.join(val_labels_dir,
                                                  "coco_output.json")
                with open(val_coco_json_path, "w") as val_coco_file:
                    json.dump(val_coco_output, val_coco_file)

            if num_val_images < len(image_files):
                train_coco_json_path = os.path.join(train_labels_dir,
                                                    "coco_output.json")
