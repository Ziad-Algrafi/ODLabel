...
def slice_and_infer(model, img, conf_threshold, log_update, image_file, slice_size):
    ...
    for i in range(0, len(slices), batch_size):
        ...
        batch_images = [slice[0] for slice in batch_slices]
        results = model(batch_images, conf=conf_threshold, imgsz=slice_size, verbose=False)

        for j, (slice, x_offset, y_offset, slice_width, slice_height) in enumerate(batch_slices):
            if results[j].boxes is not None:
                log_update.emit(f"Results for {image_file}:")
                for box in results[j].boxes:
                    ...
                    w_scaled = w_pixel
                    h_scaled = h_pixel
                    slice_detections.append((cls_id, x_scaled, y_scaled, w_scaled, h_scaled, class_name))
            else:
                log_update.emit(f"No Results for {image_file}:")

    # Concatenate slice detections with main detections list
    detections = slice_detections
