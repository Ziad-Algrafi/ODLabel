...
class YOLOGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("YOLO Auto label")
        ...
        ttk.Label(root, text="Log Run:").grid(row=3, column=0, padx=10, pady=10, sticky=tk.E)
        self.log_text = tk.Text(root, height=10, width=50, wrap=tk.WORD)
        self.log_text.grid(row=3, column=1, columnspan=2, padx=10, pady=10, sticky=tk.W)

        ttk.Label(root, text="Select Device:").grid(row=4, column=0, padx=10, pady=10)
        combobox = ttk.Combobox(root, textvariable=self.device_var, values=["cpu", "0"])
        combobox.grid(row=4, column=1, padx=10, pady=10)
        self.device_var.set("cpu")  # Set an initial value

        self.run_button = ttk.Button(root, text="Run Code", command=self.run_code, state=tk.DISABLED)
        self.run_button.grid(row=4, column=2, columnspan=2, padx=10, pady=10)
