...
def infer(model, img, conf_threshold, log_update, image_file):
    ...
    if results[0].boxes is not None:
        ...
        for box, class_id, score in zip(boxes, clss, scores):
            x, y, w, h = box
            cls_id = class_id
            ...
            detections.append((cls_id, x_scaled, y_scaled, w_scaled, h_scaled, class_name, confidence))
    else:
...
def draw_boxes_on_image(annotated_frame, detections, image_path, output_path=None, log_update=None):
    ...
    for cls_id, x_scaled, y_scaled, w_scaled, h_scaled, class_name, confidence in detections:
        ...
        cv.putText(annotated_frame, f"prob: {confidence:.2f}", (x1, y1 - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
